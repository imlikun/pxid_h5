<template>
  <div class="page">
    ...
    <div v-if="showSearch" class="bar fade-up">
      ...
      <div class="sort">
        <span :class="{ on: sort === 'dist' }" class="press chip-bounce" @click="sort = 'dist'">距离最近</span>
        <span :class="{ on: sort === 'rate' }" class="press chip-bounce" @click="sort = 'rate'">评分最高</span>
      </div>
    </div>

    <!-- 门店列表 -->
    <div class="list">
      <StoreCard v-for="(s, i) in list" :key="s.name" :store="s" :tag="i === 0 ? '购车门店' : ''" :class="['fade-up', 'stagger-' + (((i + 1) % 10))]" />
