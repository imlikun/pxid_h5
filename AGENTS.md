...
11. **密钥不进代码/文档/日志**（见 §4）。
    - 🔴 用户 token 同样**禁止落日志**（2026-09-22 修）：`server.js` 的 `[req]` 日志曾打印 `Authorization` 前 60 字符，
      32 位 hex 的 ToC 受限 token 会被**完整写入** pm2 日志（实测累积 14401 行）→ 已改为只输出「形态+长度」(`auth=toc(len=32)`)。
      **加/改日志时先问自己：这行会不会把 token/密钥写进去。**
