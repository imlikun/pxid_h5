...
- **媒体选择：原生桥优先**。`onPickImageClick` / `onPickVideoClick` 在 `bridge.isNative()` 为真时调 `pickImages` / `pickVideo`；**WebView 内 `<input type=file>` 默认不响应**（需 Flutter 配 file-chooser delegate），故原生桥是图片/视频的唯一可用通道。浏览器独立预览环境无原生桥，降级用 `<input type=file>`。
