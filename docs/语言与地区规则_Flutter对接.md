...
| `src/App.vue` | 监听 `visibilitychange`：切回前台自动执行 `initLocale()`，**改系统语言无需重启 App** |
...
| 5 | 手机系统切英文后**切回 App 前台** | 界面变英文（**无需重启 App**：H5 在切回前台时自动刷新语言） |
...
| 日期 | 说明 |
|---|---|
| 2026-08-31 | 定版。移除「地区→语言」强制映射（`REGION_LOCALE`），语言改为跟随手机系统语言；新增 URL `?lang=` / `?region=` 支持；提交 `f366c37` |
| 2026-08-31 | 补：切回前台自动刷新语言（`App.vue` 的 `visibilitychange` → `initLocale()`），**改系统语言无需重启 App**；提交 `5a17008`。副作用：i18n 因此进入主包（index 125→146 kB，gzip +7 kB） |
