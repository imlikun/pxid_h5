...
## 2026-09-12 商品详情重复左推修复

商品全屏 WebView 首次打开由 Flutter 执行原生推入，H5 `/product/:handle` 首屏不再播放 `wvPushIn`。颜色 query 和快照继续保留；单次点击仍只发一次全屏 channel，发送成功后不再 `router.push`。同一 WebView 内的商品页导航保留 H5 转场。此修订取代此前对商品全屏首屏补播500ms动画的做法。


## 2026-09-12 颜色选项与车型对齐

移除手工/哈希生成色块，颜色选项改用该Shopify颜色关联的104px缩略图；无明确关联或图片失败显示中性占位。图片imageId/variantIds明确绑定优先于alt，歧义alt不猜色。文字、缩略图、选中主图与购买变体共用颜色关联；保持全屏首屏不补播H5动画。

### 2026-09-12 详情首屏重复滑入与加载状态

- 对照 Flutter 交接文档及当前 H5，旧构建浏览器复现动态详情额外 wvPushIn；停止 App.vue 全部首屏补播，不依赖桥/全屏标识注入时机。同 WebView 内前进返回及减弱动画规则保留。
- 商品无快照冷启动改为带返回、禁用购买按钮的静态详情骨架；失败保留返回及重试，不显示整页“加载中”。已有快照和选色逻辑保留。
- 动态详情当前已为骨架，无可见“加载中”文案；原生进度圈属于 Flutter 加载遮罩。本次不能据此声称消除原生等待，也未进行手机录屏验证。
- 构建、7 项颜色单元测试、17 项商品浏览器回归、12 项首屏/桥时序检查通过。新增 tests/detail-boot.browser.mjs；先启动 :4175 预览，配置 Playwright 后用 node 执行，可用 BASE_URL 与 PLAYWRIGHT_MODULE 指定环境。

### 2026-09-12 公告详情转场闪回列表

- 线上浏览器逐帧复现：详情 enter 完成先解除 fixed，旧列表 leave 尚未完成，详情被排到列表下方，露出一帧列表；路由未返回。另发现旧导航 700ms 清理任务会误删新转场类。
- App.vue 为前进页面单独保持固定定位，等待 enter/leave 均完成再释放；补全取消/快速返回清理。导航变更取消旧清理任务，兜底不触碰仍在转场的元素。
- 四组公告逐帧检查无闪回；六组快速进入/返回/缓存复用检查、17 项商品检查及 12 项首屏检查通过。新增 tests/notice-transition.browser.mjs，使用 Playwright，BASE_URL 默认本地 :4175。未进行手机真机录屏验收。

### 2026-09-12 移除动态详情评论占位

- 按用户要求移除评论区灰色骨架及骨架替换淡入动画，评论响应后直接显示文字。
- 加载完成前不显示暂无评论；慢响应、有评论及空评论浏览器验证通过，构建通过。
