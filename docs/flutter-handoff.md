...
> H5 当前版本：`3f20a5a`（已 build 通过，已推送到 gitlab + origin 两个远端）。
> 最后更新：2026-08-14
...
- **B. 给仓库权限（推荐，对方能 pull 最新）**：`http://47.100.82.63:8099/likun/pxid_h5.git`（分支 `main`）。对方用自己 GitLab 账号 + 申请该项目权限即可 `git clone` 并随时 `git pull` 拉更新。（备选：`ssh://root@101.133.136.140/srv/sync/pxid_h5.git` 分支 `master`，需 ECS 的 ssh 访问。）
  **⚠️ 安全：上面是去掉 token 的干净地址。本地 remote 配置里实际带了访问令牌，转发时务必只用这个去 token 版本，切勿把带 `glpat-...` 的完整 URL 泄露出去。**
